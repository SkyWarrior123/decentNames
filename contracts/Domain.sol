...
contract Domain is ERC721URIStorage {
    ...
    // The Top level domain {like .eth, .xyz}
    string public tld;
    ...
    constructor(string memory _tld) payable ERC721("Decentralized Naming Service", "DNS") {
    ...
    function register(string calldata name) public payable {
        ...
        require(msg.value >= _price,"Not enough Matic spent");

        //Combine the name passed into the funtion with the TLD
        ...
        console.log("Registering %s, %s on contract with tokenId %d", name, tld, newRecordId);
        ...
        string memory json = Base64.encode(
            abi.encode(
                '{"name": "',
                _name,
                '", "description": "A domain name on the Decentralized Naming Service", "image": "data:image/svg+xml;base64,"',
    ...
    function setRecord(string calldata name, string calldata _records) public {
    ...
    function getAllNames() public view returns (string[] memory) {
        console.log("Getting all registered name from contract");
